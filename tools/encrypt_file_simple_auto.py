...
class EncryptFileSimpleAuto(Tool):
    ...
    def _invoke(self, parameters: dict[str, Any]) -> Generator[ToolInvokeMessage]:
        ...
        try:
            ...
            if file_ext == '.pdf':
                success, message = SecurityUtils.encrypt_pdf(input_path, key, input_path)
            elif file_ext == '.zip':
                success, message = SecurityUtils.encrypt_zip(input_path, key, input_path)
            elif file_ext == '.7z':
                success, message = SecurityUtils.encrypt_7z(input_path, key, input_path)
            else:
